...
namespace GabbyDialogueSample
{
    public class SampleScriptingHandler : AbstractScriptingHandler
    {
        ...
        public SampleScriptingHandler()
        {
            ...
            // Nim sample actions
            // ==================
            GameObject nimUI = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("UI/NimUI"));
            Debug.Assert(nimUI != null);
            nimUI.SetActive(false);

            UIElements nimUIElements = nimUI.GetComponent<UIElements>();
            ...
            // Vendor scripts
            // ==============
            GameObject goldUI = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("UI/GoldUI"));
            Debug.Assert(goldUI != null);
            goldUI.SetActive(false);

            UIElements goldUIElements = goldUI.GetComponent<UIElements>();
            ...
            // Lamp quest actions
            // ==================
            GameObject questUI = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("UI/QuestUI"));
            Debug.Assert(questUI != null);
            questUI.SetActive(false);

            UIElements questUIElements = questUI.GetComponent<UIElements>();
            ...
            // Tutorial actions
            // ================
            GameObject tutorialUI = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("UI/TutorialUI"));
            Debug.Assert(tutorialUI != null);
            tutorialUI.SetActive(false);

            UIElements tutorialUIElements = tutorialUI.GetComponent<UIElements>();
            Debug.Assert(tutorialUIElements != null);

            this.AddActionHandler("showSnippet", (parameters) =>
            {
                string snippetImageName = Convert.ToString(parameters[0].value);
            ...
            this.AddActionHandler("hideSnippet", (parameters) =>
            {
                tutorialUI.SetActive(false);
                return new ActionResult {handled = true, autoAdvance = true};
            });
